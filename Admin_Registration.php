...
<body>
    <div id="container">
        ...
        <form action="Admin_Registration.php" method="POST" id="form" onsubmit="return validateForm()">
            <div id="form-group">
            ...
            <div id="form-row">
                ...
                <div id="form-group">
                    <button type="submit" name="submit" onclick="validcap()">Submit</button>
                </div>
        </form>
    ...
    <script type="text/javascript">
        function validateForm() {
            ...
            var countyname = document.getElementById("County_name").value;
            var captcha = document.getElementById("textinput").value

            if (idnum == "" || idnum == null || isNaN(idnum)) {
            ...
            if (password == "" || password == null || password.length < 8 || password.length > 64) {
    ...
    <?php
    //$_POST associative array retrieve records sent to the server via the post method
    $getidnumber = $_POST['ID_number'];
    $getusername = $_POST['Username'];
    $getemail = $_POST['Email'];
    $getpassword = $_POST['Password'];
    $getphone = $_POST['Phone'];
    $getcountyname = $_POST['County_name'];

    $sql = "INSERT INTO admin_table(ID_number, Username, Email, Phone, Password, County_name)VALUES('$getidnumber','$getusername','$getemail','$getphone','$getpassword','$getcountyname')";
    //Create a new prepared statement object by calling the stmt init () method on the mysqli connection object
    $stmt = $mysqli->stmt_init();

    //Prepare the sql stmt for execution by calling the prepare method on the stmt object passing in the query string (Syntax errors on the sql will be caught) string as an argument ()
    if (!$stmt->prepare($sql)) {
        die("SQL Error:" . $mysqli->error);
    }
    //Execute statement by calling the execute()method on the stmt object
    if ($stmt->execute()) {
